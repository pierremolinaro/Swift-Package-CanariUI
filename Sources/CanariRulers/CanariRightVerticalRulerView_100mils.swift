...
private let MAIN_UNIT = CanariLength.mil (100)
private let MAIN_UNIT_STRING = "mil"
private let DISPLAY_FACTOR = 100

//--------------------------------------------------------------------------------------------------

public struct CanariRightVerticalRulerView_100mils : View {
      ...
      .overlay {
        ForEach (self.mArray_cm, id: \.self) { indexAndY in
          if self.mContext.scale > 0.5 {
            Text ("\(indexAndY.index * DISPLAY_FACTOR)").font (.system (size: 9.0))
